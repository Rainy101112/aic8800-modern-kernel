...
u8 *rwnx_build_bcn(struct rwnx_bcn *bcn, struct cfg80211_ap_update *new)
{
    u8 *buf, *pos;

    if (new->beacon.head) {
        u8 *head = kmalloc(new->beacon.head_len, GFP_KERNEL);
        ...
        bcn->head = head;
        bcn->head_len = new->beacon.head_len;
        memcpy(bcn->head, new->beacon.head, new->beacon.head_len);
    }
    if (new->beacon.tail) {
        u8 *tail = kmalloc(new->beacon.tail_len, GFP_KERNEL);
        ...
        bcn->tail = tail;
        bcn->tail_len = new->beacon.tail_len;
        memcpy(bcn->tail, new->beacon.tail, new->beacon.tail_len);
    }
                                 ...
                                 struct cfg80211_csa_settings *params)
{
    ...
    bcn = &vif->ap.bcn;
    struct cfg80211_ap_update fake_csa = {
        .beacon = params->beacon_csa, 
    };
    buf = rwnx_build_bcn(bcn, &fake_csa);
    if (!buf)
        return -ENOMEM;
    ...
    bcn_after = &csa->bcn;
    struct cfg80211_ap_update fake_update_after = {
    .beacon = params->beacon_after, 
    };
    buf = rwnx_build_bcn(bcn_after, &fake_update_after);
    if (!buf) {
        error = -ENOMEM;
        rwnx_del_csa(vif);
        goto end;
